...
| Windows 兼容作为所有后续任务的持续验收条件   | 保证任意时点从 GitHub 新克隆都不积累平台技术债   |
| 每个验证通过的小任务先提交并推送再继续       | GitHub 是 Windows 跨机器克隆与恢复的权威交付源   |
...
1. 提交并推送已通过本地门禁的 29.4。
2. 检查首次 GitHub Windows/Ubuntu 浏览器矩阵；仅在两端都通过后标记 29.4 complete，再进入 29.5 发布候选审计。
