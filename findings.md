...
| start 只在健康后发布本地进程 marker               | 复用需同时满足活 PID、有效回环 URL/port、当前 appVersion 和精确健康响应；损坏 marker 不能触发任意 URL 请求                      |
...
| 发布候选以测试行为而非手工数据检查为证据          | 迁移、恢复回滚、离线边界、中文路径和跨机密钥保留均已有真实集成/浏览器/Pester 覆盖；最终审计复跑完整门禁并逐项映射，不复制生产数据 |
| 最终 AI 验收固定为本地 fake provider              | 25 次调用日志仅含 `task` 与 `productId`，浏览器拒绝公网主机；真实 DeepSeek 配置、密钥或付费请求不是自动化发布门禁的一部分        |
...
| 本地首次托管 Chromium 门禁找不到 1234 可执行文件             | 旧验证依赖机器全局 Chrome，缓存仅有 Playwright 1228；执行文档化安装命令获取锁文件对应 1234 后，完整 6/6 浏览器门禁通过                    |
| 沙箱内 E2E 服务创建 `tsx` IPC 管道返回 `EPERM`               | 同一命令在允许本地 IPC 与监听端口的环境运行；确认是执行环境限制，不修改产品或测试绕过                                                     |
| 浏览器矩阵被 Windows 既有生产组合测试阻断                    | Windows 冷启动/扫描使真实磁盘与双启动用例耗时 5–30 秒；为该套件及清理 hook 统一设置 60 秒预算，其他测试保持默认，不使用 retry             |
| 全新克隆首次 pnpm 安装的 worker 无输出退出                    | 会话临时 pnpm 包装只复制 `pnpm.mjs`、漏掉同发行目录的 `worker.js`；改为调用已校验的完整 pnpm 11.22.0 发行目录后，同一克隆连续 setup 两次成功，确认不是仓库缺陷 |
| 最终文档提交触发的 Windows Workspace 测试超时                  | WAL 备份和跨机恢复断言实际完成需约 5.7/6.4 秒，默认 5 秒中断后才造成清理 `ENOTEMPTY`；只为真实磁盘集成套件及清理 hook 设 60 秒有界预算，不使用 retry |
